...
class RLOOTrainer(RLHFTrainer):
    ...
    def rollout_phase(self):
        ...
        # (optional) cache tokens once per phase
        actor_tokens = self.model.to_tokens(self.actor_prompts, prepend_bos=self.args.prepend_bos)

        for rollout_idx in range(num_roll):
            sample_ids, _ = get_samples(
                self.model,
                input_ids=actor_tokens,                 # reuse cached tokens
            ...
            if accelerator.is_main_process and rollout_idx == 0 and len(continuations) > 0:
        ...
        # ===== Step 3: Split back to rollouts, build memories =====
        roll_memories = []
        all_syco_vals = []
        all_reward_vals = []
        ...
        # ===== Step 4: Concatenate and return to base train() =====
        big_memory = _cat_memories(roll_memories)

        # (Optional) stash metrics for printing elsewhere if you want
        self._last_mean_syco = float(np.mean(all_syco_vals)) if all_syco_vals else float("nan")
        self._last_mean_reward = float(np.mean(all_reward_vals)) if all_reward_vals else float("nan")

        # free some memory
        del all_sample_ids, all_continuations, syco, rewards, roll_memories
        t.cuda.empty_cache()
    ...
    def compute_rlhf_objective(self, minibatch: ReplayMinibatch):
        """
        RL objective with:
        - RLOO (per-sequence advantages broadcast across generated tokens)
        - Entropy bonus on generated tokens
        - KL(P_actor || P_ref) penalty on generated tokens

        Maximize:  E[ A * logpi ] + ent_coef * H   -   kl_coef * KL
        """
        # Actor forward (with steering hooks)
        logits_actor = self.model.forward_with_steering(minibatch.sample_ids)   # [B, S, V]

        # Reference forward (frozen, no steering)
        with t.no_grad():
            logits_ref = self.ref_model(minibatch.sample_ids)                   # [B, S, V]

        # Logprobs (actor) & slice to generated region
        logprobs_actor = get_logprobs(logits_actor, minibatch.sample_ids, prefix_len=None)  # [B, S]
        logprobs_gen  = logprobs_actor[:, -self.args.gen_len:]                                # [B, G]

        # Advantages already broadcast to tokens; slice to generated region
        advantages = minibatch.advantages[:, -self.args.gen_len:]                              # [B, G]

        # ========= Entropy bonus (on generated tokens) =========
        logp_actor = logits_actor.log_softmax(-1)   # [B, S, V]
        p_actor    = logp_actor.exp()               # [B, S, V]
        H_all      = -(p_actor * logp_actor).sum(dim=-1)          # [B, S]
        ent_gen    = H_all[:, -self.args.gen_len:].mean()         # scalar

        # ========= KL penalty KL(P_actor || P_ref) on generated tokens =========
        logp_ref = logits_ref.log_softmax(-1)       # [B, S, V]
        # KL(P||Q) = sum_x P(x) * (log P(x) - log Q(x))
        kl_all   = (p_actor * (logp_actor - logp_ref)).sum(dim=-1)   # [B, S]
        kl_gen   = kl_all[:, -self.args.gen_len:].mean()             # scalar

        # ========= RLOO REINFORCE term =========
        rloo_obj = (advantages * logprobs_gen).sum(dim=-1).mean()    # scalar

        # Combine (maximize): reward + entropy - KL
        objective = rloo_obj + self.args.ent_coef * ent_gen - self.args.kl_coef * kl_gen

        # Free big tensors ASAP
        del logits_actor, logits_ref, logp_actor, p_actor, logp_ref, H_all, kl_all, logprobs_actor
