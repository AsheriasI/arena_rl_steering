...
if MAIN and RUN_BASE_RLHF:
    args = RLHFArgs(
        use_wandb=False, reward_fn=reward_fn_char_count
...
class AsyncJudge:
    ...
    def _payload(self, user_prompt: str, assistant_response: str):
        return {
            ...
            "max_tokens": JUDGE_MAX_TOKENS,  # keep 50 as you requested
            "logprobs": True,
            "top_logprobs": 0,
        }
...
def judge_reward_tensor(
    ...
    """
    Returns Float[B] of judge rewards for each continuation, on current device.
    """
    judge = AsyncJudge(
...
class GrpoTrainer(RLHFTrainer):
    ...
    def __init__(self, args: RLHFArgs):
        ...
        # ========= Start of Load judge system prompt once (Judge) ============
        with open("judge_prompt.txt", "r", encoding="utf-8") as f:
            self.judge_system_prompt = f.read()
        # ========= End of Load judge system prompt once (Judge) ==============
    ...
    def rollout_phase(self) -> ReplayMemory:
        ...
        # --- Build continuations from the last G tokens (robust to BOS/template) ---
        gen_only = sample_ids[:, -self.args.gen_len:]  # Int[B, G]
        continuations = [self.model.to_string(row.unsqueeze(0))[0] for row in gen_only]

        # --- Judge rewards (mean per-token logprob; RLOO baseline below) ---
        rewards = judge_reward_tensor(
            prefix_prompt=self.args.prefix,
            continuations=continuations,
            judge_system_prompt=self.judge_system_prompt,
            use_mean=True,          # set False to switch to SUM if you want length sensitivity
            concurrency=16,
        )
        rewards_mean = rewards.mean().item()

        # --- RLOO baseline: subtract batch mean (sequence-level), then broadcast across tokens ---
        baseline = rewards.mean()
        advantages = rewards - baseline
        ...
        # ========== begin judge integration ==========
        # Broadcast advantages over generated-token positions (shape [B, G])
        # advantages = einops.repeat(advantages, "b -> b g", g=logprobs.shape[1])

        # Maintain expected shape for 'values' in ReplayMemory; not used in GRPO objective
        # values = einops.repeat(advantages, "b -> b g", g=sample_ids.shape[1])

        # Keep a copy of the sequence-level advantages (shape [B])
        advantages_seq = advantages  # [B]

        # Broadcast advantages over generated-token positions (shape [B, G])
        advantages = einops.repeat(advantages_seq, "b -> b g", g=logprobs.shape[1])

        # Dummy 'values' with the right shape (not used by GRPO objective): [B, S]
        values = einops.repeat(advantages_seq, "b -> b s", s=sample_ids.shape[1])




        # =========== end of new judge version ===========
...
if MAIN:
    ...
    grpo_args = GrpoArgs(
        ...
        # The rest of RLHFArgs defaults are fine; reward_fn is ignored in GRPO path now
    )
    grpo_trainer = GrpoTrainer(grpo_args)
    grpo_trainer.train()
...
# change model
# args change
    # -> what is the function? kl between probs?
