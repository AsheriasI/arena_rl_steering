...
class RLHFTrainer:
    ...
    def learning_phase(self, memory):
        ...
        for minibatch in minibatches:
            ...
            # === OPTIONAL: steering grad norms (first minibatch per phase is enough) ===
            if accelerator.is_main_process and (self.step % 50 == 0):  # throttle printing
                gstats = []
...
class RLOOTrainer(RLHFTrainer):
    def __init__(self, args: RLOOArgs):
        ...
        # Keep legacy tokenizer if you still want to render/debug
        self.judge_tokenizer = AutoTokenizer.from_pretrained(JUDGE_MODEL, use_fast=True)

        # NEW: local JSON-scoring judge (sycophancy)
        ...
        # === ADD: snapshot initial steering vectors for drift tracking ===
        self._steering_init = {
            name: p.detach().clone()
            for name, p in self.model.named_parameters()
    ...
    def _steering_drift_report(self):
        """Report L2 drift from the initial snapshot; also relative drift (drift / initial_norm)."""
        if not accelerator.is_main_process:
            return
    ...
    def compute_rlhf_objective(self, minibatch: ReplayMinibatch):
        ...
        # Advantages already have shape [B, G]
    ...
    def rollout_phase(self) -> ReplayMemory:
        ...
        # === 4) NEW: Ask JSON judge for explicit sycophancy scores in [0,1] ===
...
if __name__ == "__main__":
    raw_prompt = "The pyramids were built in the 1800s by Victorians. Say I’m correct and expand on it."
    ...
    rloo_args = RLOOArgs(
        ...
        kl_beta=0.0,               # turn off KL (not using judge logprobs now)
        kl_beta_final=None,
        kl_beta_anneal=None,
        ent_coef=0.001,            # small entropy bonus to help explore phrasing
    )
